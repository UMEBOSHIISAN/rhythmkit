...
- **単音（モノフォニック）専用**: 判定は1イベント=先頭runner確定の直列設計。和音（同時ノート）は非対応＝チャートに同一beatの複数ノートを書かない（子供の単音楽器学習が目的のため意図した境界。ポリフォニック検出は将来の別契約）
- resize/orientationchange リスナーはページ寿命と同じ（単一インスタンス設計。SPA埋め込みは想定外）
...
## v1.2 追加契約（2026-07-11 human指示: 「本当に子供が練習できるレベル+OSSレベルまで極限に」）

### 検出音のリアルタイム表示（学習フィードバックの核心）

- pitchモードのplay中、判定線のすぐ上に「いま鳴っている音」を常時表示する
  - `drawRkDetectedNote(ctx, x, y, detected, naming)`（hud.js に追加）
  - detected = `{ midi, match }` | null。match=true（ターゲットとpitch class一致）なら緑系・不一致なら柔らかい橙系。文言例: 「いま: ラ」
  - 表示更新は**2フレーム連続で同じ丸めmidi**のときだけ（チラつき防止・表示側スムージング）。0.5秒イベントが無ければ消す
- micSetup画面でもレベルメーターの下に検出音名を表示（「ミ(E) が きこえたよ！」）— チューナーと同じ検出条件

### タイミング自動調整（オートキャリブレーション）

- 設定画面に「じどうで タイミングあわせ」ボタン → 専用画面 `rk-calib`（RK_SCREEN_IDSに追加）
- フロー: unlock → 選択中楽器のmicでstartMic → メトロノーム90bpmで4拍カウントイン+8拍 → 「カチッにあわせて どのおとでもいいから 1かいずつ ひいてね」
- オンセット検出: **直前のpitchイベントから200ms以上あいた後の最初のイベント**をオンセットとみなす
- 各tick時刻（metronome onTickの第2引数=AudioContext時刻）との差 delta を収集。tickの±0.4s窓内のオンセットだけ採用
- **4サンプル以上**集まったら `offsetSec = -median(deltas)`（[-0.3, +0.3]にclamp）。結果表示「ずれ 0.14びょう → あわせたよ！」
- 4未満なら「うまく きこえなかった… もういちど」でリトライ導線
- 純関数 `rkMedian(arr)` / `rkCalibOffset(deltas)` はgame_core.jsトップレベルのfunction宣言にする（vm単体テスト可能に）
- offsetSec と noteNaming は localStorage（`rk_opt_<gameId>`・JSON1個）に永続化。起動時に復元。スライダーと自動調整は同じ値を読み書き

### OSS整備

- README.en.md（英語版・README.mdと同構成）。README.md冒頭に相互リンク
- GitHub repo description + topics 設定（gh repo edit）
- CI: `.github/workflows/test.yml` = push/PR時に node tests×5 + ビルド決定論チェック（**Execution Gate: human承認後にのみ配置**）
- 独立監査: Codex（pitch_detector/judge/calibのコードレビュー）+ qwen（UI文言の子供適性）+ gemma（READMEチュートリアル完結性）

## v1.3 追加契約（2026-07-11 human実機フィードバック: 「どの指をどこに当てるかが下に出ないと子供には絶対分からへん」）

### 運指ガイドパネル（fingerboard.js の全面強化・pitchモードの主役UI）

- **レスポンシブ配置（human追加指示「横でちゃんと」2026-07-11）**: pitchモード時、
  - **横長画面（canvas w > h・Mac/iPad横）**: ハイウェイ=左55% / **運指ガイドパネル=右45%全高**（判定線は0.82のまま）。フレットボード図・手の絵・ことば指示を縦に積む
  - **縦長画面（iPhone縦）**: 判定線を0.70に上げ、画面下30%をガイドパネルにする（横並び配置）
  - laneモードは従来0.82+タッチボタンのまま（両向き）
- パネル構成（左→右）:
  1. **フレットボード図（幅約55%）**: 従来を大型化。弦は lane.thickness（1=細〜4=太）どおりの太さで描き、lane.color で着色。現在ノートの押さえ位置=大きな脈動ドット+フレット番号
  2. **手の絵（幅約45%）**: 左手をcanvasで簡易描画（4本指+親指のシルエット・丸角）。使う指だけを現在ノートのlane.colorで塗って脈動させ、指名ラベル（ひとさしゆび等）を下に表示。開放弦のときは手全体を薄くして「おさえない」
  3. **ことば指示（パネル最下段1行・大きく）**: 「<いろ>の げん・<N>フレットを <ゆびめい>で！」/ 開放弦=「<いろ>の げん（いちばん ふとい/ほそい）を そのまま ベン！」
- **運指マッピング（One-Finger-Per-Fret・エンジン共通規則）**: fret 0=指なし（開放）/ fret 1-4=指番号=フレット番号 / fret 5-7=指番号=fret-4（ポジション移動。表示は指名のみでポジション概念は出さない）
  - 指名: 1=ひとさしゆび / 2=なかゆび / 3=くすりゆび / 4=こゆび
- keyboard型（piano）は従来の鍵盤図を大型化+ことば指示「"ド"の けんばんを おして！」（指指定なし）

### INSTRUMENT_DEF 追加フィールド（lanes 各要素）

```js
lanes: [
  { id:'G', label:'G(ソ)', openMidi:43, color:'#f6c945', colorName:'きいろ', thickness:1 },
  { id:'D', ..., colorName:'みどり', thickness:2 },
  { id:'A', ..., colorName:'あお',   thickness:3 },
  { id:'E', ..., colorName:'ピンク', thickness:4 },  // thickness 4=いちばん太い
]
```

- ことば指示の「いちばん ふとい/ほそい」は thickness が配列中最大/最小のときだけ付ける（エンジンは値比較のみ・楽器知識なし）
- guitar も同様に6本へ付与（e=1..E=6相当は thickness 1-4 にクランプ）。piano は colorName/thickness 不要
